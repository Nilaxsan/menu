...
const CreateAcc=()=> {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validate password
    if (!password || !isValidPassword(password)) { // Check if password is empty or invalid
      setPasswordError("Password should have only 8 characters long and contain at least one letter, one number, and one special character.");
      return;
    } else {
      setPasswordError("");
    }
      ...
      <Grid container sx={{ height: "100vh" }}>
        ...
        <Grid item sm={4}>
          <Box
            sx={{
              width: 400,
              height: emailError || passwordError ? 575 : 525,
              marginTop: 8,
              padding: 3,
